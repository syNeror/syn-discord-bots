    ...
    async execute(member) {
      try {
        ...
        const embed = new EmbedBuilder()
          ...
          .setFooter({ text: cfg.footer.text, iconURL: member.guild.iconURL({ size: 256 }) });
        ...
        // log.json'dan kssecurity-log kanal ID'sini al
        const logData = JSON.parse(fs.readFileSync(path.join(__dirname, '../data/log.json'), 'utf8'));
        const secLogId = logData['</ KATESHİ LOG />']?.['kssecurity-log'];
        if (!secLogId) return;
        ...
        const joinLogId = logData['</ KATESHİ LOG />']?.['ksgiris-log'];
        if (joinLogId) {
          const joinCh =
